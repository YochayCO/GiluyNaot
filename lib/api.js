...
async function fetchAPI(query, { variables } = {}) {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_STRAPAPOO_URL}/graphql`, options);
...
export async function loginAndInitAPI(ctx) {
    ...
    if (jwt) {
        ...
        const { data } = await Axios.post(`${process.env.NEXT_PUBLIC_STRAPAPOO_URL}/auth/local`, loginPayload);
        jwt = data.jwt;
        user = data.user;
