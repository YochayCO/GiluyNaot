...
export function parseToVisNetwork({ user, people, companies, companyOwnerships, ownerships, relationships }) {
    const smooth = {
        type: user.role.type === 'editor' ? 'discrete' : 'dynamic',
    };

    const peopleNodes = people.map(({ id, name, picture, network_section_id }) => {
        let imageUrl;
        let shape = 'circle';
        if (picture) {
    ...
    const realCompanies = companies.filter(
        (company) => !company.group_company || company.group_company.id !== company.id,
    );

    const companyNodes = realCompanies.map(({ id, name, is_comm, picture, group_company, network_section_id }) => {
    ...
    const companyOwnershipEdges = companyOwnerships
        ...
        .map(({ id, parent, subsidiary, level }) => {
            ...
            if (subsidiary.group_company && subsidiary.group_company.id === subsidiary.id) {
    ...
    const ownershipEdges = ownerships
        ...
        .map(({ id, owner, company, level }) => {
            ...
            if (level === 'full') {
                const ownerNode = peopleNodes.find((personNode) => personNode.id === `person_${owner.id}`);
                if (ownerNode) {
