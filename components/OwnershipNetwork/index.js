...
import { parseToVisNetwork, X_DISTANCE, Y_DISTANCE, MIN_HEIGHT, MIN_WIDTH } from './utils';
...
export default class OwnershipNetwork extends React.Component {
    ...
    componentDidMount() {
        const { user, people, companies, companyOwnerships, ownerships, relationships } = this.props;
        ...
        data.nodes.forEach((node) => g.setNode(node.id, { ...node, width: 50, height: 30 }));
